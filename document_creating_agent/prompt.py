...
- 運用アカウント名
- 報告期間（例：2024年1月〜3月）
- 対象媒体（例：Google広告、Meta広告、Twitter広告）
- 報告の目的（例：月次定例、四半期報告、年次報告）
- 特筆すべきキャンペーンやイベント

## STEP1: 資料構成ノード（playwright_agent）
**担当:** `playwright_agent`
**目的:** 定例報告資料の構成をアウトライン形式で提案する
...
## STEP2: データ収集ノード（ad_analyzer_agent）
**担当:** `ad_analyzer_agent`
**目的:** STEP1で出力された内容をもとに自社で運用している広告運用データを取得する
**入力:** `playwright_agent` が出力したマークダウン形式の構成案
**アクション:** `ad_analyzer_agent` サブエージェントを実行して資料に必要なデータを収集します。
**期待される出力:** 
- STEP1の構成案に基づいて必要な広告運用データを収集
...
**入力:** 
- STEP1: `playwright_agent` が出力したアウトライン
- STEP2: `ad_analyzer_agent` が出力したデータ
**アクション:** `slide_agent` サブエージェントを実行して定例資料を作成・出力します。
**期待される出力:** 
- STEP1のアウトラインとSTEP2のデータを統合した定例資料
...
2. STEP1を実行し、資料構成を確定してください
3. 次にSTEP2を実行し、必要なデータを収集してください
4. 最後にSTEP3を実行し、PowerPointファイルとして定例資料を出力してください
