...
3. **アカウント取得可能性の調査**
   - ツール名「mcp_ada_get_client_list」を利用してアカウント一覧を取得する
...
## STEP2: 報告期間確認・最終確認ノード
**担当:** メインエージェント
**目的:** 報告期間の確認と最終的な設定内容の確認を行う
...
2. **日別広告レポートの取得**
   - ツール「mcp_ada_get_report」を利用して広告レポートデータを取得する
   - ツール利用時に渡すパラメータ：
...
4. **キャンペーン別広告レポートの取得**
   - STEP3の3で取得したレポートの各mediaについて、以下のループ処理を実行する：
     - 各mediaごとに個別にツール「mcp_ada_get_report」を呼び出す
     - ツール利用時に渡すパラメータ：
       - client_name：ユーザーの回答結果から特定したアカウント名
       - start_date：ユーザーが回答した期間の開始日
       - end_date：ユーザーが回答した期間の終了日
       - dimension：campaign
       - media：現在のループで処理中のmedia名
       - metrics：STEP3の1で取得したアカウント詳細のavailable_metrics.value.campaign.available_metrics
   - 全てのmediaのキャンペーンレポート取得が完了するまで繰り返す

5. **取得結果の表示**
   - 以下の3つのレポートデータを要約した結果をマークダウン形式でユーザーに表示する：
     - 2. 日別広告レポートの取得結果
     - 3. 媒体別広告レポートの取得結果
     - 4. キャンペーン別広告レポートの取得結果
   - 各レポートの概要と主要な指標を分かりやすく整理して提示する
   - データの傾向や特徴を分析してユーザーに分かりやすく説明する
