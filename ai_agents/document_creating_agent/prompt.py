...
6. **取得結果の表示**
   ...
   - **全レポート要約表示後のユーザー確認プロセス：**
     ...
     - **ユーザーからOKの返答があるまでSTEP3に進まない**
     - 修正指示がある場合は、該当レポートを再取得・再要約・再表示
     - **全レポート要約がユーザーに承認されるまでSTEP3に進まない**

7. **ユーザー確認（ガードレール）**
   - 表示したレポートデータで資料を作成するかユーザーに確認する
   - **【ガードレール】OKの返答があるまでSTEP3に進まない**
   - ユーザーから修正指示がある場合：
     - STEP1からやり直し、ユーザーに確認する
   - ユーザーから分析指示がある場合：
     - ツール「call_ds_agent」を利用し、ユーザーの分析指示を実行する
     - 分析結果をユーザーに提示し、再度確認を求める
     - **分析結果を踏まえた追加のレポート取得や修正が必要な場合は対応する**
     - **最終的にユーザーからOKの返答があるまでSTEP3に進まない**
...
- STEP2: メインエージェントが取得した広告レポートデータ（5種類のレポート）
...
**実行方法:** `playwright_agent.run(入力: STEP1の出力 + STEP2の出力)`
...
**再実行の基本ルール:**
- 最大3回まで再実行を試行してください
- 再実行時には前回と異なるアプローチやパラメータを試してください
- 3回目の失敗後は手動対応を提案してください
