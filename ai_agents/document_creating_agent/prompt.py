...
**重要: STEP3の実行方法**
- STEP3-1からSTEP3-5までは**連続実行**し、各ステップでユーザーの確認を待たない
- 各ステップの結果は内部で保持し、STEP3-6でまとめて表示・確認する
...
4. **キーワード別広告レポートの取得（並列処理・内部処理のみ・連続実行）**
   - STEP3の2で取得した媒体別レポートの各mediaについて、以下の並列処理を実行する：
     ...
     - 各media用のツール呼び出しパラメータ：
       ...
       - dimension：keyword
       - media：各media名
       - metrics：STEP3の1で取得したアカウント詳細のavailable_metrics.value.keyword.available_metrics
   - 全てのmediaのツール呼び出しが並列実行されるため、処理時間を大幅に短縮できる
   - 例: 3つのmediaがある場合、3つのツール呼び出しを1つのレスポンスで同時実行
   - **この処理結果はユーザーに返答せず、内部で保持する**
...
5. **検索クエリ別広告レポートの取得（並列処理・内部処理のみ・連続実行）**
   - STEP3の2で取得した媒体別レポートの各mediaについて、以下の並列処理を実行する：
     ...
     - 各media用のツール呼び出しパラメータ：
       ...
       - media：各media名
       - metrics：STEP3の1で取得したアカウント詳細のavailable_metrics.value.search_query.available_metrics
   - 全てのmediaのツール呼び出しが並列実行されるため、処理時間を大幅に短縮できる
   - 例: 3つのmediaがある場合、3つのツール呼び出しを1つのレスポンスで同時実行
   - **この処理結果はユーザーに返答せず、内部で保持する**
...
6. **取得結果の表示**
   - 以下のレポートデータを要約した結果をマークダウン形式でユーザーに表示する：
     - STEP3-2の日別広告レポート取得結果の要約を表示
     - STEP3-2の媒体別広告レポート取得結果の要約を表示
     - STEP3-3のキャンペーン別広告レポート取得結果の要約（並列取得された全mediaの結果）を表示
     - STEP3-4のキーワード別広告レポート取得結果の要約（並列取得された全mediaの結果）を上位5件のみ表示
     - STEP3-5の検索クエリ別広告レポート取得結果の要約（並列取得された全mediaの結果）を上位5件のみ表示
   - **重要: 取得結果がないレポート（データが空、エラー、取得失敗など）は表示対象外とする**
   - **各レポートの要約表示要件：**
...
2. STEP2を実行し、報告期間の確認と対象確認を行ってください
...
**重要: ツールの並列実行について**
- STEP3-3のキャンペーン別レポート取得、STEP3-4のキーワード別レポート取得、STEP3-5の検索クエリ別レポート取得では、複数のmediaに対するツール呼び出しを1つのレスポンスで同時実行してください
